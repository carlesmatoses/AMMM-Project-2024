...
void read_input() {
    ...
    // Read D
    getline(cin, line); // Reads "D = 2;"
    {
        istringstream iss(line);
        iss >> temp >> temp >> D; // Parse "D", "=", "2"
    }
    // Read n
    getline(cin, line); // Reads "n = [ 1 1 ];"
    {
        istringstream iss(line);
        iss >> temp >> temp; // Parse "n" and "="
        char bracket;
        iss >> bracket; // Read "["
        n = vector<int>(D);
        for (int p = 0; p < D; ++p) {
    ...
    // Read N
    getline(cin, line); // Reads "N = 8;"
    {
        istringstream iss(line);
        iss >> temp >> temp >> N; // Parse "N", "=", "8"
    }
    
    // Read d
    getline(cin, line); // Reads "d = [1 1 1 1 2 2 2 2 ];"
    {
        istringstream iss(line);
        iss >> temp >> temp; // Parse "d" and "="
        char bracket;
        iss >> bracket; // Read "["
        d = vector<int>(N);
        for (int i = 0; i < N; ++i) {
    ...
    // Read m
    getline(cin, line); // Reads "m = ["
    getline(cin, line); // Read the first row
    m = vector<vector<double>>(N, vector<double>(N));
    for (int i = 0; i < N; ++i) {
        istringstream iss(line);
        char bracket;
        iss >> bracket; // Read "[" for the row
        for (int j = 0; j < N; ++j) {
            iss >> m[i][j];
        }
        getline(cin, line); // Read next row or "]"
    }

}

void read_input_original() {
    cin >> D;
    n = vector<int>(D);
