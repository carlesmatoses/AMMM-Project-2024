...
pair<double, vector<bool>> solve_greedy(int i) {
    ...
    // Randomly select the first commission member
    // srand(time(0)); // Seed the random number generator
    // int first_member = rand() % N;
    int first_member = i;
    commission[first_member] = true;
    candidates[first_member] = false;
    ...
    return {average_compatibility, commission};
}
...
int main() {
    while (cin >> D) {
        ...
        vector<bool> bestCommission;
        double bestCompatibility = 0.0;

        int total_members_needed = accumulate(n.begin(), n.end(), 0);
        for (int i = 0; i < total_members_needed; ++i) {
            auto [average_compatibility, commission] = solve_greedy(i);
            if (average_compatibility > bestCompatibility) {
                bestCompatibility = average_compatibility;
